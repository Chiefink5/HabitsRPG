...
const ICON_SETS={All:["⛏","🪓","🗡","🛡","🧱","💎","🪙","⚒️","💪","💧","🧠","🏃","😴","🧘","❤️","🎸","🎵","🎤","🎨","✍️","💼","📚","🧾","🖥️","🔥","🍔","🧃","🍕","🍜","☕","🍫","🎮","🎬","🎲","🚀","👑","⭐","⚡","🌀","🔒","✅","🎯","📈","🏠","🧼","🧹","🌙","🥤","📦","💰"]};
const $=s=>document.querySelector(s), $$=s=>Array.from(document.querySelectorAll(s));
const KEY="skillforge_v5_drastic";
const uid=p=>`${p}_${Math.random().toString(36).slice(2,10)}`, todayStr=()=>new Date().toISOString().slice(0,10), nowIso=()=>new Date().toISOString();
const esc=v=>String(v??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;");
const xpReq=(b,l)=>Math.round(b*Math.pow(1.5,Math.max(0,l-1)));
let rewardFilter="All",questFilter="Active",skillSearch="",skillSort="recent",currentIconTarget=null,audioCtx=null;
function seed(){const g=uid("skill"),f=uid("skill"),l=uid("skill"),wh=uid("habit");return{meta:{lastOpenedDate:todayStr()},settings:{theme:"overworld"},wallet:{coins:18,totalCoinsEarned:18,totalCoinsSpent:0},stats:{overallLevel:1,habitsCompletedToday:0,xpEarnedToday:0,coinsEarnedToday:0,rewardsRedeemed:0},analytics:{positiveHits:0,negativeHits:0,totalXpEarned:0,totalXpLost:0,totalHabitsApplied:0,totalQuestsDone:0},skills:[{id:g,name:"Guitar",icon:"🎸",description:"Keep the fingers honest",level:1,xp:0,baseXp:50,xpRequired:50,expanded:true,lastUsedAt:nowIso()},{id:f,name:"Fitness",icon:"💪",description:"Body grind",level:1,xp:0,baseXp:50,xpRequired:50,expanded:true,lastUsedAt:nowIso()},{id:l,name:"Learning",icon:"🧠",description:"Level the brain",level:1,xp:0,baseXp:50,xpRequired:50,expanded:false,lastUsedAt:nowIso()}],habits:[{id:uid("habit"),skillId:g,name:"Practice Bends",icon:"🔥",description:"Accuracy and pitch",xpReward:15,coinReward:3,dailyCap:3,timesCompletedToday:0,totalCompletions:0,streak:0,bestStreak:0,lastCompletedDate:null,isNegative:false},{id:uid("habit"),skillId:g,name:"Warmups",icon:"🎯",description:"Loosen up",xpReward:10,coinReward:2,dailyCap:2,timesCompletedToday:0,totalCompletions:0,streak:0,bestStreak:0,lastCompletedDate:null,isNegative:false},{id:wh,skillId:f,name:"Drink Water",icon:"💧",description:"Hydrate",xpReward:8,coinReward:1,dailyCap:5,timesCompletedToday:0,totalCompletions:0,streak:0,bestStreak:0,lastCompletedDate:null,isNegative:false},{id:uid("habit"),skillId:l,name:"Read 10 Pages",icon:"📚",description:"Just read the damn thing",xpReward:12,coinReward:2,dailyCap:2,timesCompletedToday:0,totalCompletions:0,streak:0,bestStreak:0,lastCompletedDate:null,isNegative:false},{id:uid("habit"),skillId:l,name:"Doom Scroll",icon:"🌀",description:"Negative habit example",xpReward:8,coinReward:1,dailyCap:4,timesCompletedToday:0,totalCompletions:0,streak:0,bestStreak:0,lastCompletedDate:null,isNegative:true}],rewards:[{id:uid("reward"),name:"1 Hour Gaming",icon:"🎮",category:"Fun",cost:30,repeatable:true,note:"No guilt run"},{id:uid("reward"),name:"Energy Drink",icon:"🧃",category:"Food",cost:15,repeatable:true,note:"One can only"},{id:uid("reward"),name:"Movie Night",icon:"🎬",category:"Rest",cost:45,repeatable:true,note:"Kick back"}],quests:[{id:uid("quest"),type:"habit_count",title:"Complete Drink Water 7 times",targetId:wh,targetValue:7,rewardCoins:20,completed:false,completedAt:null},{id:uid("quest"),type:"skill_level",title:"Reach Guitar level 2",targetId:g,targetValue:2,rewardCoins:25,completed:false,completedAt:null}],activityLog:[]}}
let state=load();
function load(){try{const raw=localStorage.getItem(KEY);const d=raw?JSON.parse(raw):seed();if(d.meta.lastOpenedDate!==todayStr()){d.habits.forEach(h=>h.timesCompletedToday=0);d.stats.habitsCompletedToday=0;d.stats.xpEarnedToday=0;d.stats.coinsEarnedToday=0;d.meta.lastOpenedDate=todayStr()}return d}catch(e){return seed()}}
...
function log(text){state.activityLog.unshift({id:uid("log"),text,createdAt:nowIso()});state.activityLog=state.activityLog.slice(0,30)}
function lvl(){const total=state.skills.reduce((s,x)=>s+x.level,0);state.stats.overallLevel=Math.max(1,Math.floor(total/Math.max(1,state.skills.length))||1)}
function tone(freq=440,d=.08,type="sine",vol=.03){try{audioCtx ||= new(window.AudioContext||window.webkitAudioContext)();const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.frequency.value=freq;o.type=type;g.gain.value=vol;o.connect(g);g.connect(audioCtx.destination);o.start();g.gain.exponentialRampToValueAtTime(.0001,audioCtx.currentTime+d);o.stop(audioCtx.currentTime+d)}catch(e){}}
const ding=()=>[440,660,880].forEach((f,i)=>setTimeout(()=>tone(f,.12,"triangle",.04),i*90)),good=()=>tone(560,.06,"square",.02),bad=()=>tone(220,.08,"sawtooth",.018),questTone=()=>[530,700].forEach((f,i)=>setTimeout(()=>tone(f,.09,"triangle",.035),i*70));
function toast(html,kind="good"){const el=document.createElement("div");el.className=`toast ${kind}`;el.innerHTML=html;$("#toastStack").appendChild(el);setTimeout(()=>{el.style.opacity="0";el.style.transform="translateY(8px)";setTimeout(()=>el.remove(),180)},2400)}
function applyTheme(t){document.body.classList.remove("theme-overworld","theme-nether","theme-end");document.body.classList.add(`theme-${t}`);state.settings.theme=t;save()}
function rank(l){return l>=10?"Legend":l>=7?"Overboss":l>=5?"Veteran":l>=3?"Runner":"Fresh Spawn"} function avatar(l){return l>=10?"🦾":l>=7?"🧙":l>=5?"⚔️":l>=3?"🧢":"🙂"}
function setTab(tab){$$(".tab-btn").forEach(b=>b.classList.toggle("active",b.dataset.tab===tab));$$(".tab-panel").forEach(p=>p.classList.remove("active"));$(`#${tab}Tab`).classList.add("active")}
function qProg(q){if(q.type==="habit_count"){const h=getHabit(q.targetId);return{current:h?h.totalCompletions:0,target:q.targetValue}} if(q.type==="skill_level"){const s=getSkill(q.targetId);return{current:s?s.level:0,target:q.targetValue}} if(q.type==="streak_goal"){const h=getHabit(q.targetId);return{current:h?h.streak:0,target:q.targetValue}} return{current:state.wallet.totalCoinsEarned,target:q.targetValue}}
function qType(t){return({habit_count:"Habit Count",skill_level:"Skill Level",streak_goal:"Streak Goal",coin_goal:"Coin Goal"})[t]||t} function qIcon(q){return({habit_count:"📜",skill_level:"⭐",streak_goal:"🔥",coin_goal:"🪙"})[q.type]||"📜"}
function finishQuests(){state.quests.forEach(q=>{if(q.completed)return;const p=qProg(q);if(p.current>=p.target){q.completed=true;q.completedAt=nowIso();state.wallet.coins+=q.rewardCoins;state.wallet.totalCoinsEarned+=q.rewardCoins;state.analytics.totalQuestsDone++;log(`Quest complete — ${q.title} (+${q.rewardCoins} coins)`);toast(`Quest complete<br><strong>${esc(q.title)}</strong><br>+${q.rewardCoins} coins`);questTone()}})}
function hitHabit(id){const h=getHabit(id),s=getSkill(h.skillId);if(!h||!s)return;if(h.timesCompletedToday>=h.dailyCap){toast(`Cap reached for <strong>${esc(h.name)}</strong>`,"bad");bad();return}const neg=h.isNegative,xp=neg?Math.ceil(h.xpReward*1.5):h.xpReward,coin=neg?Math.ceil(h.coinReward*1.5):h.coinReward;h.timesCompletedToday++;h.totalCompletions++;state.analytics.totalHabitsApplied++;const last=h.lastCompletedDate;if(last!==todayStr()){if(last){const diff=Math.floor((new Date(todayStr())-new Date(last))/(1000*60*60*24));h.streak=diff===1?h.streak+1:1}else h.streak=1;h.bestStreak=Math.max(h.bestStreak,h.streak)}h.lastCompletedDate=todayStr();s.lastUsedAt=nowIso();if(neg){s.xp=Math.max(0,s.xp-xp);state.wallet.coins=Math.max(0,state.wallet.coins-coin);state.analytics.negativeHits++;state.analytics.totalXpLost+=xp;log(`Negative habit — ${h.name} (-${xp} XP / -${coin} coins)`);toast(`Negative habit hit<br><strong>${esc(h.name)}</strong><br>-${xp} XP / -${coin} coins`,"bad");bad()}else{s.xp+=xp;state.wallet.coins+=coin;state.wallet.totalCoinsEarned+=coin;state.stats.habitsCompletedToday++;state.stats.xpEarnedToday+=xp;state.stats.coinsEarnedToday+=coin;state.analytics.positiveHits++;state.analytics.totalXpEarned+=xp;log(`+${xp} XP / +${coin} coins — ${h.name}`);toast(`+${xp} XP / +${coin} coins<br><strong>${esc(h.name)}</strong>`);good()}while(s.xp>=s.xpRequired){s.xp-=s.xpRequired;s.level++;s.xpRequired=xpReq(s.baseXp,s.level);log(`Level up — ${s.name} Lv ${s.level}`);toast(`Level up!<br><strong>${esc(s.name)}</strong> is now Lv ${s.level}`);ding()}lvl();finishQuests();save();render()}
function undoHabit(id){const h=getHabit(id),s=getSkill(h.skillId);if(!h||!s)return;if(h.timesCompletedToday<=0){toast(`Nothing to undo for <strong>${esc(h.name)}</strong>`,"bad");return}const neg=h.isNegative,xp=neg?Math.ceil(h.xpReward*1.5):h.xpReward,coin=neg?Math.ceil(h.coinReward*1.5):h.coinReward;h.timesCompletedToday=Math.max(0,h.timesCompletedToday-1);h.totalCompletions=Math.max(0,h.totalCompletions-1);state.analytics.totalHabitsApplied=Math.max(0,state.analytics.totalHabitsApplied-1);if(neg){s.xp+=xp;state.wallet.coins+=coin;state.analytics.negativeHits=Math.max(0,state.analytics.negativeHits-1);state.analytics.totalXpLost=Math.max(0,state.analytics.totalXpLost-xp);log(`Undo negative habit — ${h.name}`)}else{s.xp=Math.max(0,s.xp-xp);state.wallet.coins=Math.max(0,state.wallet.coins-coin);state.stats.habitsCompletedToday=Math.max(0,state.stats.habitsCompletedToday-1);state.stats.xpEarnedToday=Math.max(0,state.stats.xpEarnedToday-xp);state.stats.coinsEarnedToday=Math.max(0,state.stats.coinsEarnedToday-coin);state.analytics.positiveHits=Math.max(0,state.analytics.positiveHits-1);state.analytics.totalXpEarned=Math.max(0,state.analytics.totalXpEarned-xp);log(`Undo habit — ${h.name}`)}good();save();render()}
function buyReward(id){const r=getReward(id);if(!r)return;if(state.wallet.coins<r.cost){toast(`Need ${r.cost-state.wallet.coins} more coins for <strong>${esc(r.name)}</strong>`,"bad");bad();return}state.wallet.coins-=r.cost;state.wallet.totalCoinsSpent+=r.cost;state.stats.rewardsRedeemed++;log(`Redeemed reward — ${r.name} (-${r.cost} coins)`);toast(`Redeemed<br><strong>${esc(r.name)}</strong><br>-${r.cost} coins`);questTone();if(!r.repeatable)state.rewards=state.rewards.filter(x=>x.id!==id);save();render()}
function sSort(a,b){if(skillSort==="name")return a.name.localeCompare(b.name);if(skillSort==="level")return(b.level-a.level)||(b.xp-a.xp);if(skillSort==="progress")return(b.xp/b.xpRequired)-(a.xp/a.xpRequired);return new Date(b.lastUsedAt)-new Date(a.lastUsedAt)}
...
function modal(title,body){
  ...
  $("#modalHost").innerHTML=`<div class="modal"><div class="modal-head"><h3>${title}</h3><button class="close-btn" data-close>×</button></div><div class="modal-body">${body}</div></div>`;
}
function closeModal(){
...
function iconModal(){modal("Pick Icon",`<div class="button-row">${Object.keys(ICON_SETS).map(cat=>`<button class="filter-pill" data-icon-cat="${cat}">${cat}</button>`).join("")}</div><div class="icon-grid" id="iconGrid"></div>`);renderIconGrid("All")}
function renderIconGrid(cat){$("#iconGrid").innerHTML=ICON_SETS[cat].map(i=>`<button class="icon-btn" data-pick-icon="${i}">${i}</button>`).join("")}
function options(){return{skills:state.skills.map(s=>`<option value="${s.id}">${esc(s.icon)} ${esc(s.name)}</option>`).join(""),habits:state.habits.map(h=>`<option value="${h.id}">${esc(h.icon)} ${esc(h.name)}</option>`).join("")}}
function openSkill(id=null){const s=id?getSkill(id):null;modal(s?"Edit Skill":"Add Skill",`<label>Name<input id="fsn" value="${esc(s?.name||"")}"></label><label>Base XP Goal<input id="fsb" type="number" min="10" value="${s?.baseXp||50}"></label><label>Description<input id="fsd" value="${esc(s?.description||"")}"></label>${picker("fsi",s?.icon||"🎸")}<div class="button-row"><button class="btn primary" data-save-skill="${s?.id||""}">Save Skill</button>${s?`<button class="btn danger" data-del-skill="${s.id}">Delete</button>`:""}</div>`)}
function openHabit(id=null,prefill=null){const h=id?getHabit(id):null,o=options();modal(h?"Edit Habit":"Add Habit",`<label>Name<input id="fhn" value="${esc(h?.name||"")}"></label><label>Parent Skill<select id="fhs">${o.skills}</select></label><label>XP Value<input id="fhx" type="number" min="1" value="${h?.xpReward||10}"></label><label>Coin Value<input id="fhc" type="number" min="0" value="${h?.coinReward||2}"></label><label>Daily Cap<input id="fhd" type="number" min="1" value="${h?.dailyCap||1}"></label><label>Description<input id="fhm" value="${esc(h?.description||"")}"></label><label><span class="muted">Negative Habit</span> <input id="fhnb" type="checkbox" ${h?.isNegative?"checked":""}></label>${picker("fhi",h?.icon||"🔥")}<div class="button-row"><button class="btn primary" data-save-habit="${h?.id||""}">Save Habit</button>${h?`<button class="btn danger" data-del-habit="${h.id}">Delete</button>`:""}</div>`);setTimeout(()=>{$("#fhs").value=h?.skillId||prefill||state.skills[0]?.id||""},0)}
function openReward(id=null){const r=id?getReward(id):null;modal(r?"Edit Reward":"Add Reward",`<label>Title<input id="frn" value="${esc(r?.name||"")}"></label><label>Cost<input id="frc" type="number" min="1" value="${r?.cost||25}"></label><label>Category<select id="frg">${["Fun","Food","Rest","Purchase","Custom"].map(x=>`<option ${r?.category===x?"selected":""}>${x}</option>`).join("")}</select></label><label>Note<input id="frt" value="${esc(r?.note||"")}"></label><label><span class="muted">Repeatable</span> <input id="frr" type="checkbox" ${r?.repeatable!==false?"checked":""}></label>${picker("fri",r?.icon||"🎮")}<div class="button-row"><button class="btn primary" data-save-reward="${r?.id||""}">Save Reward</button>${r?`<button class="btn danger" data-del-reward="${r.id}">Delete</button>`:""}</div>`)}
function autoTitle(type,targetId,targetValue){if(type==="habit_count"){const h=getHabit(targetId);return`Complete ${h?.name||"habit"} ${targetValue} times`}if(type==="skill_level"){const s=getSkill(targetId);return`Reach ${s?.name||"skill"} level ${targetValue}`}if(type==="streak_goal"){const h=getHabit(targetId);return`Maintain ${h?.name||"habit"} streak for ${targetValue} days`}return`Earn ${targetValue} coins`}
function openQuest(id=null){const q=id?state.quests.find(x=>x.id===id):null,o=options();modal(q?"Edit Quest":"Add Quest",`<label>Quest Type<select id="fqt"><option value="habit_count" ${q?.type==="habit_count"?"selected":""}>Habit Count</option><option value="skill_level" ${q?.type==="skill_level"?"selected":""}>Skill Level</option><option value="streak_goal" ${q?.type==="streak_goal"?"selected":""}>Streak Goal</option><option value="coin_goal" ${q?.type==="coin_goal"?"selected":""}>Coin Goal</option></select></label><div id="fqh"><label>Target Habit<select id="fqhx">${o.habits}</select></label></div><div id="fqs"><label>Target Skill<select id="fqsx">${o.skills}</select></label></div><label>Target Value<input id="fqtv" type="number" min="1" value="${q?.targetValue||5}"></label><label>Reward Coins<input id="fqr" type="number" min="1" value="${q?.rewardCoins||20}"></label><label>Custom Title<input id="fqn" value="${esc(q?.title||"")}"></label><div class="button-row"><button class="btn primary" data-save-quest="${q?.id||""}">Save Quest</button>${q?`<button class="btn danger" data-del-quest="${q.id}">Delete</button>`:""}</div>`);setTimeout(()=>{$("#fqt").value=q?.type||"habit_count";if($("#fqhx"))$("#fqhx").value=q?.targetId||state.habits[0]?.id||"";if($("#fqsx"))$("#fqsx").value=q?.targetId||state.skills[0]?.id||"";qVis()},0)}
function qVis(){const t=$("#fqt")?.value;if(!t)return;$("#fqh").style.display=(t==="habit_count"||t==="streak_goal")?"block":"none";$("#fqs").style.display=t==="skill_level"?"block":"none"}
function openSettings(){modal("Settings",`<div class="theme-grid"><button class="theme-card ${state.settings.theme==="overworld"?"active":""}" data-theme="overworld"><div class="theme-card-title">Overworld Merchant</div><div class="theme-card-sub">emerald / stone / village stall layout</div></button><button class="theme-card ${state.settings.theme==="nether"?"active":""}" data-theme="nether"><div class="theme-card-title">Nether Crate</div><div class="theme-card-sub">red / gold / boxier crate-heavy view</div></button><button class="theme-card ${state.settings.theme==="end"?"active":""}" data-theme="end"><div class="theme-card-title">End Bazaar</div><div class="theme-card-sub">purple / cyan / side-nav bazaar look</div></button></div><button class="btn secondary" id="exportSave">Export Save</button><label class="btn secondary" style="display:flex;justify-content:center"><input type="file" id="importSave" accept=".json,application/json" style="display:none">Import Save</label><button class="btn danger" id="resetAll">Reset Everything</button>`)}
function saveSkill(id){const p={name:$("#fsn").value.trim(),baseXp:Number($("#fsb").value),description:$("#fsd").value.trim(),icon:$("#fsi").value};if(!p.name)return;if(id){const s=getSkill(id);Object.assign(s,p);s.xpRequired=xpReq(s.baseXp,s.level);log(`Updated skill — ${s.name}`)}else{state.skills.push({id:uid("skill"),...p,level:1,xp:0,xpRequired:p.baseXp,expanded:true,lastUsedAt:nowIso()});log(`Added skill — ${p.name}`)}save();closeModal();render()}
function saveHabit(id){const p={skillId:$("#fhs").value,name:$("#fhn").value.trim(),xpReward:Number($("#fhx").value),coinReward:Number($("#fhc").value),dailyCap:Number($("#fhd").value),description:$("#fhm").value.trim(),icon:$("#fhi").value,isNegative:$("#fhnb").checked};if(!p.name||!p.skillId)return;if(id){Object.assign(getHabit(id),p);log(`Updated habit — ${p.name}`)}else{state.habits.push({id:uid("habit"),...p,timesCompletedToday:0,totalCompletions:0,streak:0,bestStreak:0,lastCompletedDate:null});log(`Added habit — ${p.name}`)}save();closeModal();render()}
function saveReward(id){const p={name:$("#frn").value.trim(),cost:Number($("#frc").value),category:$("#frg").value,note:$("#frt").value.trim(),repeatable:$("#frr").checked,icon:$("#fri").value};if(!p.name)return;if(id){Object.assign(getReward(id),p);log(`Updated reward — ${p.name}`)}else{state.rewards.push({id:uid("reward"),...p});log(`Added reward — ${p.name}`)}save();closeModal();render()}
function saveQuest(id){const type=$("#fqt").value,targetId=type==="skill_level"?$("#fqsx").value:type==="coin_goal"?null:$("#fqhx").value,targetValue=Number($("#fqtv").value),title=$("#fqn").value.trim()||autoTitle(type,targetId,targetValue),p={type,targetId,targetValue,rewardCoins:Number($("#fqr").value),title};if(id){Object.assign(state.quests.find(x=>x.id===id),p,{completed:false,completedAt:null});log(`Updated quest — ${title}`)}else{state.quests.push({id:uid("quest"),...p,completed:false,completedAt:null});log(`Added quest — ${title}`)}save();closeModal();render()}
function delSkill(id){const s=getSkill(id);if(!s||!confirm(`Delete skill "${s.name}" and its habits?`))return;state.skills=state.skills.filter(x=>x.id!==id);state.habits=state.habits.filter(h=>h.skillId!==id);state.quests=state.quests.filter(q=>q.targetId!==id);log(`Deleted skill — ${s.name}`);save();closeModal();render()}
function delHabit(id){const h=getHabit(id);if(!h||!confirm(`Delete habit "${h.name}"?`))return;state.habits=state.habits.filter(x=>x.id!==id);state.quests=state.quests.filter(q=>q.targetId!==id);log(`Deleted habit — ${h.name}`);save();closeModal();render()}
function delReward(id){const r=getReward(id);if(!r||!confirm(`Delete reward "${r.name}"?`))return;state.rewards=state.rewards.filter(x=>x.id!==id);log(`Deleted reward — ${r.name}`);save();closeModal();render()}
